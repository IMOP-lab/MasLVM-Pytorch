...
def create_dataloaders(name_im_gt_list, my_transforms=[], batch_size=1, training=False):
    ...
    if training:
        ...
        for i in range(len(name_im_gt_list)):
            ...
            dataloader = DataLoader(gos_dataset, batch_size=batch_size, sampler=sampler, drop_last=False, num_workers=num_workers_)
...
class RandomHFlip(object):
    ...
    def __call__(self,sample):
        imidx, image, label, shape, name =  sample['imidx'], sample['image'], sample['label'], sample['shape'],sample['name']
        ...
        return {'imidx':imidx,'image':image, 'label':label, 'shape':shape, 'name': name}
...
class Resize(object):
    ...
    def __call__(self,sample):
        ...
        return {'imidx':imidx,'image':image, 'label':label, 'shape':torch.tensor(self.size),'name':name}
...
class LargeScaleJitter(object):
    ...
    def __call__(self, sample):
        imidx, image, label, image_size, name =  sample['imidx'], sample['image'], sample['label'], sample['shape'],sample['name']
        ...
        return {'imidx':imidx,'image':image, 'label':label, 'shape':torch.tensor(image.shape[-2:]),'name':name}
...
class DataPreprocess():
    def __init__(self):
        ...
        self.gt = []
    def Default(self):
        return self.im, self.gt
    def preprocess_with_name(self, im, gt, dataset_name):
        self.im = im
        self.gt = gt
        if self.gt.max() == 1:
            self.gt = self.gt * 255

        fun = getattr(self, dataset_name, self.Default)
        return fun()
...
class OnlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt = np.copy(gt)
        im,gt = self.preprocess.preprocess_with_name(im,gt,'Default')
        ...
        if self.eval_ori_resolution:
            ...
            sample['name'] = self.dataset["im_name"][idx]
