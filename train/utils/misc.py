...
def mask_dice(pred_label, label):

    pred_label_bin = (pred_label > 0)[0].int()
    label_bin = (label > 128)[0].int()
    intersection = (label_bin * pred_label_bin).sum()
    pred_label_size = pred_label_bin.sum()
    label_size = label_bin.sum()
    dice = (2. * intersection + 1e-6) / (pred_label_size + label_size + 1e-6)
