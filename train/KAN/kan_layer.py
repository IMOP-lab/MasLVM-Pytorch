...
class KANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 3, 1).reshape(batch * height * width, channels).to(self.device)
        
        x = torch.einsum('ij,k->ikj', x, torch.ones(self.out_dim, device=self.device)).reshape(batch * height * width, self.size).permute(1, 0)
        preacts = x.permute(1, 0).clone().reshape(batch * height * width, self.out_dim, self.in_dim)
        ...
        y = y.permute(1, 0)
        postspline = y.clone().reshape(batch * height * width, self.out_dim, self.in_dim)
        y = self.scale_base.unsqueeze(dim=0) * base + self.scale_sp.unsqueeze(dim=0) * y
        y = self.mask[None, :] * y
        postacts = y.clone().reshape(batch * height * width, self.out_dim, self.in_dim)
        y = torch.sum(y.reshape(batch * height * width, self.out_dim, self.in_dim), dim=2)
        
        # Reshape back to (batch, out_dim, height, width)
        y = y.reshape(batch, height, width, self.out_dim).permute(0, 3, 1, 2)
