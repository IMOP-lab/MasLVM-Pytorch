...
class CCSABlock(nn.Module):
    ...
    def forward(self, x, f1):
        if self.channel_att:
            x_ca = self.channel_attention2(x,f1)
            x = self.m_sum(x, x_ca)   
        if self.spatial_att:
            x_sa = self.spatial_attention2(x,f1)
            x = self.m_sum(x, x_sa)   
        return x

    def channel_attention2(self, x,f1):
    ...
    def spatial_attention2(self, x, f1):
        x_c = self.m_apply(x, self.spatial_norm)
        x_cin = self.cat(*f1)
        x_in = [[x_cin, x_cin, v] for v in x_c]        
        x_att = self.m_apply(x_in, self.s_attention)
...
class DCA(nn.Module):
    def __init__(self,
        ...
        self.patch_avg = nn.ModuleList([PoolEmbedding(
                                                    ...
                                                    for _ in features])
        self.patch_avg2 = nn.ModuleList([PoolEmbedding(
                                                    pooling = nn.AdaptiveAvgPool2d,            
                                                    patch=patch, 
                                                    )
                                                    for _ in features])                        
    ...
    def forward(self, raw, f1):
        x = self.m_apply(raw, self.patch_avg) 
        f1 = self.m_apply(f1, self.patch_avg2) 
